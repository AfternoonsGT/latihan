@extends('master')
@section('content')
    <form action="{{ route('reviews.update', $review->id) }}" method="post" class="form-floating">
        @csrf
        @method('PUT')
    ...
    @enderror
        ...
        <div class="form-floating mb-3">
            <input type="text" class="form-control" id="floatingInput" placeholder="Asia Heritage" name="reviewer_name" value="{{$review->reviewer_name}}" @error ('reviewer_name') is-invalid @enderror" value="{{ old('reviewer_name')}}" required>
            @error('reviewer_name')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
        ...
        <div class="form-floating mb-3">
            <textarea name="comment" id="" class="form-control" placeholder="Comment">{{$review->comment}}</textarea>
